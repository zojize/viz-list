...
describe('scope cleanup', () => {
  it('marks function parameters dead on return', () => {
    ...
    `)
    // After foo returns, the stack cells for x and y should be dead
    const liveCells = [...context.memory.cells.values()].filter(c => !c.dead && c.region === 'stack')
    // Only main's variables should remain (none in this case)
  ...
  it('marks struct field cells dead on delete', () => {
    ...
    `)
    // After delete, all 3 cells (header + data + next) should be dead
  ...
  it('cleans up all locals after insertBack+freeAll', () => {
    ...
    `)
    // After program completes, all cells should be dead (including main's locals)
